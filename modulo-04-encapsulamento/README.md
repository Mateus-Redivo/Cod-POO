...
Seus objetos até aqui expunham os atributos só através de métodos que você escolhia (como `exibirDados()`). Agora vamos formalizar isso: `private` nos atributos, e getters/setters como a única porta de entrada e saída dos dados. Ainda sem validação — isso é assunto do próximo módulo.
